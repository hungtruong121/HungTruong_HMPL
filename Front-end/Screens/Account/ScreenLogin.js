...
import { View, StyleSheet, Text, TouchableOpacity, ImageBackground, AsyncStorage } from 'react-native';
import ParallaxScrollView from 'react-native-parallax-scroll-view';

export default class ScreenAccountLogin extends React.Component {
    _retrieveData = async (navigate) => {
        try {
            const value = await AsyncStorage.getItem('token');
            if (value !== null) {
                console.log('Chuyen trang yeu cau dang nhap')
                navigate('Đăng nhập')
            }
        } catch (error) {
