...
export default class Detail extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            url : this.props.navigation.getParam('url', 'default')
        };
    }

    handleIncreaseView = async () => {
        const { navigation } = this.props;
        const token = await AsyncStorage.getItem('token');
        const id = navigation.getParam('id', 'default');
        fetch(`https://hml-project.herokuapp.com/api/dish/${id}`);
        if(token){
            fetch(`https://hml-project.herokuapp.com/api/history?id=${id}&token=${token}`);
        }
    ...
    //     const { navigation } = this.props;
    //     const id = navigation.getParam('id', 'default');
    //     fetch(`https://hml-project.herokuapp.com/api/history?${id}`);
    // }
    render = () =>
     {
                ...
                <View style={styles.container} key="1">
                    ...
                    <ScrollView style={styles.footer}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between'}}>
                            <Text style={styles.textDetail}>{this.props.navigation.state.params.ingredient}</Text>
                            {/* <TouchableOpacity style={styles.btnBuy}>
                                <Text style={{color: 'white'}}>Mua ngay</Text>
                            </TouchableOpacity> */}
                        </View>
                        <View style={styles.box}>
                            <TouchableOpacity style={styles.button} onPress={() => { navigate('Hướng dẫn', 
                            { 
                                name: this.state.name,
                                url: this.state.url,
                            }), this.handleIncreaseView() }}>
